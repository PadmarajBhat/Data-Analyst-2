...
#initial plot
ggplot(aes(alcohol), data=rw_df) + geom_histogram(binwidth = .1)
```
```{r echo=FALSE}
...
#initial plot
ggplot(aes(sulphates), data=rw_df) + geom_histogram(binwidth = .01)
```
```{r echo=FALSE}
...
ggplot(aes(alcohol, sulphates, color=quality), data=rw_df_1) + 
  geom_smooth(method = 'gam' ) + geom_point()+ facet_wrap(~quality, scales = "free_y")
```
```{r echo=FALSE}
...
ggplot(aes(fixed.acidity, citric.acid, color=quality), data=rw_df_1) + 
...
```{r echo=FALSE, Multivariate_Plots_alcohol_facidity_woo}
